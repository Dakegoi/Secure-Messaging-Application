...
class Database:
    ...
    def list_users(self) -> List[dict]:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT username, password_hash, wrapped_keys, public_payload, created_at, email, reset_code, reset_code_created_at FROM users"
            ).fetchall()
    ...
    def get_user(self, username: str) -> Optional[dict]:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT username, password_hash, wrapped_keys, public_payload, created_at, email, reset_code, reset_code_created_at FROM users WHERE username = ?",
                (username,),
            ).fetchone()
    ...
    def get_user_by_email(self, email: str) -> Optional[dict]:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT username, password_hash, wrapped_keys, public_payload, created_at, email, reset_code, reset_code_created_at FROM users WHERE email = ?",
                (email,),
            ).fetchone()
        return self._row_to_user(row) if row else None
    ...
    def create_user(self, user: dict) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO users (username, password_hash, wrapped_keys, public_payload, created_at, email, reset_code, reset_code_created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    user["username"],
